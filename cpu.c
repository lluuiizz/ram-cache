...
#include "headers/program_reader.h"
#include <stdlib.h>
#include <stdio.h>
...
void iniciar (instrucao_str *programa,cpu *processor, RAM *RAM, caches *cache) {
	...
	while (processor->opcode != HALT){
		...
		switch (processor->opcode) {
			...
			case SOMA: // SOMA
				printf("Estamos somando %d + %d que resulta em ", processor->reg1->palavras[inst->addr1.endpalavra], processor->reg2->palavras[inst->addr2.endpalavra]);
				...
				printf("%d\n", processor->reg3->palavras[inst->addr3.endpalavra]);
			...
			case INTERRUPCAO:
				...
				if (interrompido == 0){
					...
					interrompido = 1;
					ler_programa("tratador_interrupcao.txt", RAM, processor, cache);
